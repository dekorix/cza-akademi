(function(root){
  ...
  function setCaregiver(id,value){const q=root.E3_BANK?.caregiver?.find(item=>item.id===id);if(!q)return {ok:false,error:'Geçersiz bakımveren maddesi.'};if(!q.responseScale.includes(value))return {ok:false,error:'Geçersiz bakımveren yanıtı.'};state.caregiver[id]=value;save();return {ok:true}}
