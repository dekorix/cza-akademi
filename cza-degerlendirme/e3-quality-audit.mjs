...
for(const f of ['e3-bank-v3.js','e3-bank-v3-foundation.js','e3-engine-v3.js'])vm.runInContext(fs.readFileSync(new URL(`./${f}`,import.meta.url),'utf8'),c,{filename:f});
...
for(const d of bank.domains){
  ...
  for(const age of [36,39,42,45,47]){
    ...
    assert.ok(modalities.size>=3,`${d.id}/${age}: yöntem çeşitliliği düşük`);
...
console.log('E3_V3_QUALITY_AUDIT_OK',JSON.stringify({domains:bank.domains.length,tasks:bank.allTasks.length,caregiver:bank.caregiver.length}));
