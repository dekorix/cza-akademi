  ...
  return <div className="min-w-0">
    ...
    {phase==='summary'&&record&&<section className="space-y-5">
      <div className="rounded-2xl border border-border bg-white p-6 md:p-8"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-secondary text-primary"><BookOpenCheck size={25}/></span><p className="eyebrow mt-5 text-primary">Çalışma tamamlandı</p><h2 className="mt-3 text-3xl font-semibold tracking-tight">Bir adımı daha görünür kıldın.</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">{lesson.title} · Sonuç ve izlediğin yol birlikte değerlendirildi. Bu özet yalnızca bu denemeyi anlatır; ustalık veya seviye onayı vermez.</p>
        ...
        <div className="mt-6 flex flex-wrap gap-3"><Button className="h-11 px-5" onClick={startGuided}><RotateCcw/> Dersi yeniden çalış</Button>{!last&&<Button variant="outline" className="h-11 px-5" onClick={onNext}>Sonraki dersi keşfet <ArrowRight/></Button>}<a href="/educator?tab=teaching" className="inline-flex h-11 items-center gap-2 px-3 text-sm font-semibold text-primary">Eğitimci görünümü <ChevronRight size={16}/></a></div>
      </div>
...
export default function Learn() {
  ...
  return <div className="min-h-screen bg-background">
    <header className="bg-[#182739] px-5 text-white md:px-9"><div className="mx-auto flex min-h-20 max-w-[1400px] flex-wrap items-center justify-between gap-3 py-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#d8eeac] text-xs font-black text-[#182739]">CZA</span><div><p className="text-sm font-semibold">Soroban öğrenme yolu</p><p className="mt-0.5 text-[10px] tracking-widest text-[#aebdcd]">EGZERSİZ AKADEMİSİ</p></div></div><a href="/" className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#668092] bg-[#243b50] px-4 text-sm font-bold text-white shadow-sm transition hover:bg-[#31516c]"><ArrowLeft size={18}/> Çalışma merkezim</a></div></header>
