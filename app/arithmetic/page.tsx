...
export default function ArithmeticPage() {
  ...
  const [fingerMode, setFingerMode] = useState<Extract<PressMode, 'guided' | 'free'>>('guided');
  ...
  function finger(side: 'left' | 'right', fingerName: FingerName) {
    ...
    const transition = transitionFinger(currentHand, fingerName, fingerMode);
    ...
    if (autoCorrected) void record('finger_rule_violation', { fingerId: fingerName, attemptedFinger: fingerName, currentDigit: currentDigit.valid ? currentDigit.value : null, ruleMode: fingerMode, autoCorrected: true, timestamp: new Date().toISOString() });
  }

  function changeFingerMode(nextMode: Extract<PressMode, 'guided' | 'free'>) {
    if (nextMode === fingerMode) return;
    if (nextMode === 'guided') {
      if (!readHand(left).valid) setLeft(emptyHand());
      if (!readHand(right).valid) setRight(emptyHand());
    }
    setFingerMode(nextMode);
    void record('finger_rule_mode_change', { from: fingerMode, to: nextMode, invalidStateNormalized: nextMode === 'guided' && (!readHand(left).valid || !readHand(right).valid) });
  }
  ...
  return <main className="min-h-screen bg-[#f8f4eb] p-4">
    <div className="mx-auto max-w-7xl">
      ...
      <div className="grid gap-4 lg:grid-cols-[190px_minmax(340px,1fr)_minmax(380px,460px)]">
        ...
        <section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-center text-lg font-bold text-muted-foreground">İŞLEM</h2><div className="mx-auto mt-5 w-fit min-w-[clamp(12rem,26vw,22rem)] border-b-4 border-[#315a83] pb-4">{sequence.map((item,stepIndex) => <ExerciseNumberDisplay key={`${current?.id}-${stepIndex}`} operator={item.operator} value={item.value} size="large" animate className="w-full" />)}</div><div className="mx-auto mt-7 max-w-sm"><div className="min-h-20 rounded-xl border-2 border-[#315a83] bg-[var(--exercise-number-surface)] p-3"><ExerciseNumberDisplay value={answer || '—'} size="compact" className="mx-auto w-fit"/></div>{settings.inputMode !== 'keyboard' && <div className="mt-4 grid grid-cols-3 gap-2">{[1,2,3,4,5,6,7,8,9].map(digit => <button key={digit} className="arithmetic-key" onClick={() => addDigit(String(digit))}>{digit}</button>)}<button className="arithmetic-key arithmetic-key-danger" aria-label="Girilen cevabı temizle" onClick={() => {setAnswer('');void record('answer_clear',{});}}><X className="mx-auto" size={28}/></button><button className="arithmetic-key" onClick={() => addDigit('0')}>0</button><button className="arithmetic-key arithmetic-key-confirm" aria-label="Yanıtı kontrol et ve gönder" onClick={() => void submit(answer === '' ? null : Number(answer),'answer')}><CheckCircle2 className="mx-auto"/></button></div>}</div></section>
        {settings.toolMode !== 'none' && <aside className="rounded-2xl border bg-white p-4 shadow-sm"><div className="mb-4 flex rounded-xl border bg-[#f7f5ef] p-1">{settings.toolMode !== 'finger' && <button className={`flex flex-1 items-center justify-center gap-2 rounded-lg p-3 font-bold ${activeTool === 'soroban' ? 'bg-primary text-white shadow-sm' : 'text-muted-foreground'}`} onClick={() => {setActiveTool('soroban');void record('tool_switch',{tool:'soroban'});}}><Grid3X3 size={19}/> Soroban</button>}{settings.toolMode !== 'soroban' && <button className={`flex flex-1 items-center justify-center gap-2 rounded-lg p-3 font-bold ${activeTool === 'finger' ? 'bg-primary text-white shadow-sm' : 'text-muted-foreground'}`} onClick={() => {setActiveTool('finger');void record('tool_switch',{tool:'finger'});}}><Hand size={19}/> Parmak</button>}</div>
          {activeTool === 'soroban' && settings.toolMode !== 'finger' ? <div><Soroban value={sorobanValue} digits={settings.sorobanRodCount} onChange={value => {setSorobanValue(value);void record('soroban_bead_move',{value});}}/><Button variant="outline" className="mt-5 w-full" onClick={() => {setSorobanValue(0);void record('soroban_reset',{before:sorobanValue});}}><RotateCcw/> Sorobanı sıfırla</Button></div> : <div><p className="mb-2 text-center text-sm font-bold text-[#714323]">✋ PARMAK DESTEĞİ</p><div className="mb-4 grid grid-cols-2 rounded-xl border bg-[#fff8ef] p-1"><button className={`flex min-h-11 items-center justify-center gap-2 rounded-lg font-bold transition ${fingerMode === 'guided' ? 'bg-[#176e5e] text-white shadow-sm' : 'text-[#6b6258]'}`} onClick={() => changeFingerMode('guided')}>{fingerMode === 'guided' && <CheckCircle2 size={17}/>} Kurallı</button><button className={`flex min-h-11 items-center justify-center gap-2 rounded-lg font-bold transition ${fingerMode === 'free' ? 'bg-[#315a83] text-white shadow-sm' : 'text-[#6b6258]'}`} onClick={() => changeFingerMode('free')}>{fingerMode === 'free' && <CheckCircle2 size={17}/>} Serbest</button></div><div className="grid grid-cols-2"><FingerHand side="left" pattern={left} interactive onFinger={fingerName => finger('left',fingerName)}/><FingerHand side="right" pattern={right} interactive onFinger={fingerName => finger('right',fingerName)}/></div><p className="mt-3 text-center text-xs text-muted-foreground">{fingerMode === 'guided' ? 'Kurallı mod geçerli parmak sırasını otomatik korur.' : 'Serbest modda parmakları bağımsız değiştirebilirsin.'}</p></div>}
        </aside>}
      </div>
      <div className="mt-4 flex justify-end"><Button variant="outline" disabled={saving} onClick={finishEarly}>Çalışmayı bitir ve kaydet</Button></div>
    </div>
  </main>;
}
