...
import { ArrowLeft, CheckCircle2, Delete, Loader2, RotateCcw, Settings2, Timer } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { FingerHand } from '@/components/finger-hand';
import { Soroban } from '@/components/soroban';
import { emptyHand, readHands, transitionFinger, type FingerName, type HandPattern } from '@/lib/finger-engine';
...
function DigitSelector({ title, values, onChange }: { title: string; values: number[]; onChange: (values: number[]) => void }) {
  return <fieldset className="rounded-2xl border border-border bg-white p-5"><legend className="px-2 text-lg font-bold">{title}</legend><div className="mt-2 grid grid-cols-9 gap-2">{digitOptions.map(digit => <label key={digit} className={`grid min-h-12 cursor-pointer place-items-center rounded-xl border text-lg font-bold transition active:translate-y-0.5 ${values.includes(digit) ? 'border-primary bg-primary text-white shadow-sm' : 'bg-[#fbf7ee]'}`}><input className="sr-only" type="checkbox" checked={values.includes(digit)} onChange={() => onChange(values.includes(digit) ? values.filter(value => value !== digit) : [...values,digit].sort())} />{digit}</label>)}</div><div className="mt-3 flex gap-2"><Button type="button" variant="outline" size="sm" onClick={() => onChange(digitOptions)}>Tümünü seç</Button><Button type="button" variant="outline" size="sm" onClick={() => onChange([])}>Temizle</Button></div></fieldset>;
}
...
export default function ArithmeticPage() {
  ...
  function finger(side: 'left' | 'right', fingerName: FingerName) {
    ...
    const next = transitionFinger(currentHand, fingerName, 'free').state;
  ...
  if (phase === 'results') return <main className="min-h-screen bg-emerald-50 p-5"><section className="mx-auto max-w-4xl rounded-3xl border bg-white p-7 shadow-lg"><p className="eyebrow text-primary">SEANS TAMAMLANDI</p><h1 className="mt-2 text-3xl font-bold">Çalışma Performansı</h1><p className="mt-2 text-lg">{results.length} sorudan {summary.correct} tanesini doğru cevapladın.</p><div className="my-7 grid grid-cols-2 gap-3 md:grid-cols-5">{[['Soru',results.length],['Doğru',summary.correct],['Yanlış',summary.wrong],['Cevapsız',summary.timeout],['Başarı',`%${summary.accuracy}`]].map(([label,value]) => <div key={label} className="rounded-xl bg-[#edf5f0] p-4 text-center"><strong className="text-3xl">{value}</strong><p>{label}</p></div>)}</div><p className="text-center text-lg font-bold">Ortalama cevaplama süresi: {summary.average} saniye</p><div className="mt-6 flex flex-wrap gap-2">{results.map((result,resultIndex) => <button key={result.question.id} className={`grid h-12 w-12 place-items-center rounded-xl font-bold text-white ${result.status === 'correct' ? 'bg-emerald-600' : result.status === 'timeout' ? 'bg-slate-500' : 'bg-amber-600'}`} aria-label={`${resultIndex+1}. soru ayrıntısını aç`} onClick={() => setSelectedResult(result)}>{resultIndex+1}</button>)}</div>{selectedResult && <div role="dialog" aria-modal="true" aria-label="Soru ayrıntısı" className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4"><section className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-2xl font-bold">Soru ayrıntısı</h2><Button variant="outline" onClick={() => setSelectedResult(null)}>Kapat</Button></div><div className="mt-5 rounded-xl bg-[#f7f5ef] p-5 font-mono text-2xl font-bold">{selectedResult.question.initialValue}{selectedResult.question.steps.map(step => <div key={step.order}>{step.operator}{step.operand}</div>)}</div><dl className="mt-5 grid grid-cols-2 gap-3"><div><dt>Doğru cevap</dt><dd className="text-2xl font-bold">{selectedResult.question.finalAnswer}</dd></div><div><dt>Verilen cevap</dt><dd className="text-2xl font-bold">{selectedResult.answer ?? 'Cevapsız'}</dd></div><div><dt>Süre</dt><dd className="font-bold">{(selectedResult.elapsedMs/1000).toFixed(1)} sn</dd></div><div><dt>Kullanılan araç</dt><dd className="font-bold">{selectedResult.tool === 'soroban' ? 'Soroban' : selectedResult.tool === 'finger' ? 'Parmak' : 'Zihinden'}</dd></div></dl></section></div>}<div className="mt-7 flex gap-3"><Button variant="outline" onClick={() => setPhase('settings')}>Ayarlar</Button><Button onClick={() => setPhase('instructions')}>Yeni çalışma</Button><a href="/" className="inline-flex min-h-11 items-center rounded-xl border px-5 font-semibold">Bitir</a></div></section></main>;
