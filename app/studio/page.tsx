          ...
          <div ref={stageRef} style={active && (runConfig.mode === 'flash' || runConfig.mode === 'audio') ? { backgroundColor: runConfig.backgroundColor ?? '#ffffff', color: runConfig.textColor ?? '#111827' } : undefined} className={`overflow-hidden rounded-2xl border ${darkStage ? 'border-[#273c51] bg-[#182739] text-white' : 'border-border bg-white'}`}>
            ...
            <div className="flex min-h-[430px] flex-col items-center justify-center px-5 py-9 md:min-h-[475px]">
              ...
              {phase === 'answer' && current && <form className="w-full max-w-md text-center" onSubmit={e => {e.preventDefault();submit();}}>
                ...
                {runConfig.mode === 'soroban-write' && <><p className="focus-number mb-5 text-5xl font-bold">{current.answer}</p><Soroban value={abacusValue} digits={runConfig.digits} onChange={setAbacusValue} /></>}
              ...
              {phase === 'feedback' && feedbackAttempt && <div className="w-full max-w-xl text-center"><CheckCircle2 size={48} className={`mx-auto mb-5 ${feedbackAttempt.correct ? 'text-[#80aa94]' : 'text-[#b2733d]'}`} /><h2 className={`text-2xl font-semibold ${feedbackAttempt.correct ? 'text-[#39715c]' : 'text-[#a65c31]'}`}>{feedbackAttempt.correct ? 'DOĞRU' : 'YANLIŞ'}</h2><div className="mt-5 grid grid-cols-2 gap-3 text-left"><div className="rounded-xl border border-border p-4"><p className="text-[11px] uppercase tracking-wide opacity-55">Doğru cevap</p><p className="focus-number mt-2 text-3xl font-bold">{feedbackAttempt.expected}</p></div><div className="rounded-xl border border-border p-4"><p className="text-[11px] uppercase tracking-wide opacity-55">Girdiğin cevap</p><p className="focus-number mt-2 text-3xl font-bold">{feedbackAttempt.given}</p></div></div><p className="mt-4 text-xs opacity-60">{round + 1 < runConfig.rounds ? 'Bir sonraki soru otomatik açılacak.' : 'Seans özeti hazırlanıyor.'}</p></div>}
