...
export default function Studio() {
  ...
  const openQuestion = useCallback(() => {
    if (timedReading) setPhase('stimulus');
  ...
  async function start() {
    try {
      ...
      setQuestions(generated); setRunConfig({...defaultConfig,...config, pool:[...config.pool], additionPool:[...(config.additionPool ?? config.pool)], subtractionPool:[...(config.subtractionPool ?? config.pool)]}); setRound(0); setTerm(0); setAttempts([]); setFeedbackAttempt(null); setAnswer(''); setAbacusValue(0); setPaused(false); setError(''); setSaved(false); setAnswerRemainingMs(0); setPhase(config.countdownEnabled === false ? (['finger-read','soroban-read'].includes(config.mode) ? 'stimulus' : ['flash','audio'].includes(config.mode) ? 'sequence' : 'answer') : 'countdown'); if (config.countdownEnabled === false && !['finger-read','soroban-read','flash','audio'].includes(config.mode)) { responseStart.current = performance.now(); answerStartedAt.current = new Date().toISOString(); } setSettingsOpen(false); sessionStart.current = Date.now();
  ...
  async function submit(timedOut = false) {
    ...
    const attempt = { sequence: current.sequence, expected: current.answer, given, correct: !timedOut && given === current.answer, timeout: timedOut, elapsedMs, responseLatencyMs: elapsedMs, stimulusDurationMs: timedReading ? runConfig.presentationDurationMs ?? 1000 : runConfig.interval * 1000, answerDurationLimitMs: runConfig.answerDurationMs ?? 0, presentationStartedAt: presentationStartedAt.current || undefined, presentationEndedAt: presentationEndedAt.current || undefined, answerStartedAt: answerStartedAt.current || undefined, answeredAt: new Date(completedAt).toISOString() };
          ...
          <div ref={stageRef} style={active && (runConfig.mode === 'flash' || runConfig.mode === 'audio') ? { backgroundColor: runConfig.backgroundColor ?? '#ffffff', color: runConfig.textColor ?? '#111827' } : undefined} className={`overflow-hidden rounded-2xl border ${darkStage ? 'border-[#273c51] bg-[#182739] text-white' : 'border-border bg-white'}`}>
            ...
            <div className="flex min-h-[430px] flex-col items-center justify-center px-5 py-9 md:min-h-[475px]">
              ...
              {phase === 'answer' && current && <form className="w-full max-w-md text-center" onSubmit={e => {e.preventDefault();submit();}}>
                ...
                {timedReading && <p className="mb-5 text-xs opacity-55">Görsel kapandı. Cevabını hatırladığın sayı üzerinden ver.{(runConfig.answerDurationMs ?? 0) > 0 ? ` ${Math.ceil(answerRemainingMs/1000)} saniyen var.` : ''}</p>}
                {runConfig.mode === 'soroban-write' && <><p className="focus-number mb-5 text-5xl font-bold">{current.answer}</p><Soroban value={abacusValue} digits={runConfig.rods ?? 5} onChange={setAbacusValue} /></>}
              ...
              {phase === 'feedback' && feedbackAttempt && <div className="w-full max-w-xl text-center"><CheckCircle2 size={48} className={`mx-auto mb-5 ${feedbackAttempt.correct ? 'text-[#80aa94]' : 'text-[#b2733d]'}`} /><h2 className={`text-2xl font-semibold ${feedbackAttempt.correct ? 'text-[#39715c]' : 'text-[#a65c31]'}`}>{feedbackAttempt.correct ? 'DOĞRU' : feedbackAttempt.timeout ? 'SÜRE DOLDU' : 'YANLIŞ'}</h2><div className="mt-5 grid grid-cols-2 gap-3 text-left"><div className="rounded-xl border border-border p-4"><p className="text-[11px] uppercase tracking-wide opacity-55">Doğru cevap</p><p className="focus-number mt-2 text-3xl font-bold">{feedbackAttempt.expected}</p></div><div className="rounded-xl border border-border p-4"><p className="text-[11px] uppercase tracking-wide opacity-55">Girdiğin cevap</p><p className="focus-number mt-2 text-3xl font-bold">{feedbackAttempt.timeout ? 'Cevap verilmedi' : feedbackAttempt.given}</p></div></div><p className="mt-4 text-xs opacity-60">{round + 1 < runConfig.rounds ? 'Bir sonraki soru otomatik açılacak.' : 'Seans özeti hazırlanıyor.'}</p></div>}
