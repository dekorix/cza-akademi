...
export default function Studio() {
  ...
  useEffect(() => {
    ...
    return registerAcademyTools([
      ...
      { name: 'configure_exercise', title: 'Deneme egzersizini ayarla', description: 'Hazır durumdaki deneme ayarlarını değiştirir; seans başlatmaz veya öğrenciye atama yapmaz.', inputSchema:{type:'object',properties:{mode:{enum:['soroban-read','soroban-write','flash','audio']},digits:{type:'integer',minimum:1,maximum:5},minDigits:{type:'integer',minimum:1,maximum:5},maxDigits:{type:'integer',minimum:1,maximum:5},terms:{type:'integer',minimum:2,maximum:30},rounds:{type:'integer',minimum:1,maximum:100},interval:{type:'number',minimum:0,maximum:10},operation:{enum:['add','subtract','mixed']},pool:{type:'array',items:{type:'integer',minimum:1,maximum:9},minItems:1},additionPool:{type:'array',items:{type:'integer',minimum:1,maximum:9},minItems:1},subtractionPool:{type:'array',items:{type:'integer',minimum:1,maximum:9},minItems:1},maxValue:{type:'integer',minimum:1,maximum:99999},firstWithinMax:{type:'boolean'},showNumbers:{type:'boolean'},fontSize:{type:'integer',minimum:48,maximum:180},backgroundColor:{type:'string'},textColor:{type:'string'},freePractice:{type:'boolean'}},additionalProperties:false}, annotations:{readOnlyHint:false,untrustedContentHint:false}, execute: async input => {
        ...
        const allowed = ['mode','digits','minDigits','maxDigits','terms','rounds','interval','operation','pool','additionPool','subtractionPool','maxValue','firstWithinMax','showNumbers','fontSize','backgroundColor','textColor','freePractice'];
  ...
  async function start() {
    try {
      ...
      setQuestions(generated); setRunConfig({...config, pool:[...config.pool], additionPool:[...(config.additionPool ?? config.pool)], subtractionPool:[...(config.subtractionPool ?? config.pool)]}); setRound(0); setTerm(0); setCountdown(3); setAttempts([]); setFeedbackAttempt(null); setAnswer(''); setAbacusValue(0); setPaused(false); setError(''); setSaved(false); setPhase('countdown'); setSettingsOpen(false); sessionStart.current = Date.now();
  ...
  async function submit() {
    ...
    const attempt = { sequence: current.sequence, expected: current.answer, given, correct: given === current.answer, elapsedMs: completedAt-responseStart.current, stimulusDurationMs: runConfig.interval * 1000 };
    ...
    try {
      ...
      const all = [...attempts, attempt]; setAttempts(all); setFeedbackAttempt(attempt); setSaved(saveDemoResult({ id: sessionId, at: new Date().toISOString(), config: runConfig, attempts: all, durationMs: completedAt-sessionStart.current })); setSync('ready'); setPhase('feedback');
  ...
  function nextQuestion() { setRound(v=>v+1); setTerm(0); setCountdown(3); setAnswer(''); setAbacusValue(0); setFeedbackAttempt(null); setPhase('countdown'); }
  useEffect(() => {
  ...
  }, [phase, round, runConfig.rounds, sessionId]);
        ...
        <div className="space-y-5">
          <div ref={stageRef} style={active && (runConfig.mode === 'flash' || runConfig.mode === 'audio') ? { backgroundColor: runConfig.backgroundColor ?? '#ffffff', color: runConfig.textColor ?? '#111827' } : undefined} className={`overflow-hidden rounded-2xl border ${darkStage ? 'border-[#273c51] bg-[#182739] text-white' : 'border-border bg-white'}`}>
            ...
            <div className="flex min-h-[430px] flex-col items-center justify-center px-5 py-9 md:min-h-[475px]">
              ...
              {phase === 'countdown' && <div className="text-center"><p className="mb-5 text-sm opacity-60">{paused ? 'Çalışma duraklatıldı' : 'Hazır ol'}</p><div className="mx-auto grid h-32 w-32 place-items-center rounded-full border-4 border-[#8fba9e] shadow-[inset_0_0_0_12px_rgba(143,186,158,.12)]"><p className="focus-number text-[76px] font-semibold">{paused ? 'Ⅱ' : countdown}</p></div><p className="mt-4 text-xs opacity-60">Soru {round+1} / {runConfig.rounds}</p></div>}
              {phase === 'sequence' && current && <div className="text-center"><p className="mb-6 text-xs opacity-60">{paused ? 'Duraklatıldı · sürdürünce aynı sayı tekrar gösterilir' : `${term+1}. sayı / ${current.sequence.length}`}</p>{runConfig.mode === 'audio' || runConfig.showNumbers === false ? <div className="flex h-[150px] items-center justify-center">{runConfig.mode === 'audio' ? <AudioLines size={100} strokeWidth={1.2} className="text-[#89b8a1]" /> : <span className="text-sm opacity-55">Sayı gizli · zihninden takip et</span>}</div> : <div aria-live="off" className="focus-number min-h-[150px] font-semibold leading-none" style={{ fontSize: `${runConfig.fontSize ?? 95}px` }}>{paused ? 'Ⅱ' : `${term > 0 && current.sequence[term] > 0 ? '+' : ''}${current.sequence[term]}`}</div>}<p className="mt-5 text-xs opacity-60">{runConfig.mode === 'audio' ? 'Dinle ve zihninde hesapla.' : runConfig.showNumbers === false ? 'Sayıyı zihninde canlandır.' : 'İşlem işaretini takip et.'}</p>{runConfig.interval === 0 && !paused && <Button className="mt-7 h-10 px-5" onClick={advance}>{term+1 < current.sequence.length ? 'Sonraki sayı' : 'Cevaba geç'} <ChevronRight /></Button>}</div>}
              ...
              {phase === 'feedback' && feedbackAttempt && <div className="w-full max-w-xl text-center"><CheckCircle2 size={48} className={`mx-auto mb-5 ${feedbackAttempt.correct ? 'text-[#80aa94]' : 'text-[#b2733d]'}`} /><h2 className={`text-2xl font-semibold ${feedbackAttempt.correct ? 'text-[#39715c]' : 'text-[#a65c31]'}`}>{feedbackAttempt.correct ? 'DOĞRU' : 'YANLIŞ'}</h2><div className="mt-5 grid grid-cols-2 gap-3 text-left"><div className="rounded-xl border border-border p-4"><p className="text-[11px] uppercase tracking-wide opacity-55">Doğru cevap</p><p className="mt-2 text-2xl font-semibold">{feedbackAttempt.expected}</p></div><div className="rounded-xl border border-border p-4"><p className="text-[11px] uppercase tracking-wide opacity-55">Girdiğin cevap</p><p className="mt-2 text-2xl font-semibold">{feedbackAttempt.given}</p></div></div><p className="mt-4 text-xs opacity-60">{round + 1 < runConfig.rounds ? 'Bir sonraki soru otomatik açılacak.' : 'Seans özeti hazırlanıyor.'}</p></div>}
              {phase === 'finished' && <div className="w-full max-w-xl text-center"><p className="eyebrow text-[#6a9e82]">Seans tamamlandı</p><h2 className="mt-3 text-3xl font-semibold tracking-tight">Her çalışma yeni bir ipucu.</h2><div className="my-7 grid grid-cols-3 divide-x divide-border/30"><div><p className="text-3xl font-semibold">{result.correct}/{result.total}</p><p className="mt-2 text-xs opacity-60">Doğru yanıt</p></div><div><p className="text-3xl font-semibold">%{result.accuracy}</p><p className="mt-2 text-xs opacity-60">Bu seans doğruluğu</p></div><div><p className="text-3xl font-semibold">{Math.round(attempts.reduce((s,a)=>s+a.elapsedMs,0)/Math.max(1,attempts.length)/1000)} sn</p><p className="mt-2 text-xs opacity-60">Ort. yanıt süresi</p></div></div><p className="text-sm leading-6 opacity-65">{result.accuracy >= 80 ? 'Bu ayarlarda iyi bir temel oluşturdun. Hızı artırmadan önce birkaç tutarlı seans daha tamamla.' : 'Hızını azaltıp aynı basamakta tekrar çalışabilirsin. Yanlışların, bir sonraki çalışmanın yönünü gösterir.'}</p><div className="mt-6 flex justify-center gap-3"><Button className="h-10 px-4" onClick={reset}><RotateCcw /> Yeni seans</Button><a href="/educator" className="inline-flex items-center gap-2 rounded-lg border border-border px-4 text-xs">Eğitimci görünümü <ArrowRight size={14}/></a></div><p className="mt-5 text-[11px] text-[#39715c]">Tüm yanıtlar merkezi sisteme kaydedildi ve seans kapatıldı.{saved?' Bu cihazda ayrıca kısa süreli bir yedek tutuldu.':''}</p></div>}
            </div>
            {active && <div className={`border-t px-6 py-4 ${darkStage ? 'border-white/10' : 'border-border'}`}><div className="mb-3 flex items-center justify-between"><span className="text-xs opacity-60">{attempts.length} / {runConfig.rounds} yanıt</span><div className="flex gap-2">{['countdown','sequence'].includes(phase) && <Button variant="ghost" size="sm" onClick={()=>setPaused(v=>!v)}>{paused ? <Play /> : <Pause />}{paused ? 'Sürdür' : 'Duraklat'}</Button>}<Button variant="outline" size="sm" disabled={savingAttempt||sync==='saving'} onClick={finishSession}>Bitir ve kaydet</Button></div></div><Progress value={attempts.length/runConfig.rounds*100} aria-label="Seans ilerlemesi" /><p className="mt-3 text-[10px] opacity-55">Bitirdiğinde o ana kadar verdiğin bütün cevaplar eğitimci kaydında kalır.</p></div>}
          </div>
