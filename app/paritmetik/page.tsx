...
import { ArrowLeft, CheckCircle2, Delete, Hand, LogOut, RefreshCw, Wifi, WifiOff } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { FingerHand } from '@/components/finger-hand';
import { emptyHand, numberPattern, readHands, type FingerName, type HandPattern } from '@/lib/finger-engine';
...
export default function ParitmetikPage() {
  ...
  async function touch(side: 'left' | 'right', finger: FingerName) {
    ...
    const next = { ...current, [finger]: !current[finger] };
    ...
    try {
      await core('interaction', {
        ...
        eventType: 'finger_touch',
        screenArea: side,
        finger,
        previousState: current[finger],
        newState: next[finger],
        elapsedMs: Math.round(performance.now() - questionStarted.current),
        payload: { targetNumber: target, engine: 'strict-v06', surface: 'academy' },
      });
      setSync('ready');
    } catch {
      setSync('error');
  ...
  async function checkAnswer() {
    ...
    const handResult = readHands(left, right);
    ...
    if (mode === 'read') {
      ...
      correct = numericAnswer === target;
      if (!correct) {
        const reversed = Number(String(target).padStart(2, '0').split('').reverse().join(''));
        if (numericAnswer === reversed) {
          errorType = 'F09';
          message = 'Onlar ve birler alanları ters.';
        } else if (target % 10 === 0 && numericAnswer === Math.floor(target / 10)) {
          errorType = 'F10';
    ...
    try {
      await core('attempt', {
        ...
        payload: {
          ...
          targetNumber: target,
          tensDigit: Math.floor(target / 10),
          onesDigit: target % 10,
          ...
          handMode: 'two',
          pressMode: mode === 'press' ? 'semi' : null,
          learningMode: 'practice',
          difficultyLevel: 8,
          attemptNumber: 1,
          metadata: { engine: 'strict-v06', surface: 'academy' },
        },
      });
      setSync('ready');
    } catch {
  ...
  const livePattern = readHands(left, right);
  const visiblePattern = numberPattern(target);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-[1280px] px-5 py-7 md:px-9">
        ...
        <section className="paritmetik-stage rounded-3xl border border-border bg-white p-5 shadow-sm md:p-8">
          ...
          {mode === 'press' && <div className="focus-number mb-3 text-center text-6xl font-bold text-[#173f75] md:text-7xl">{String(target).padStart(2, '0')}</div>}
          {mode === 'read' && <div className="mb-3 min-h-12 text-center text-4xl font-semibold tracking-[.12em] text-[#173f75]">{answer || '—'}</div>}

          <div className="mx-auto grid max-w-[760px] grid-cols-2 items-end gap-2 md:gap-8">
            <div><p className="mb-1 text-center text-xs font-semibold text-muted-foreground">ONLAR</p><FingerHand side="left" pattern={mode === 'read' ? visiblePattern.left : left} interactive={mode === 'press'} onFinger={(finger) => touch('left', finger)} /></div>
            <div><p className="mb-1 text-center text-xs font-semibold text-muted-foreground">BİRLER</p><FingerHand side="right" pattern={mode === 'read' ? visiblePattern.right : right} interactive={mode === 'press'} onFinger={(finger) => touch('right', finger)} /></div>
          </div>
          ...
          {mode === 'read' ? (
            ...
            <div className="mx-auto mt-3 max-w-xl space-y-3"><div className={`rounded-xl p-3 text-sm ${livePattern.valid ? 'bg-blue-50 text-blue-900' : 'bg-red-50 text-red-800'}`}><strong>Canlı analiz:</strong> {livePattern.valid ? `Desen geçerli · ${String(livePattern.value).padStart(2, '0')}` : livePattern.message}</div><div className="flex gap-3"><Button variant="outline" className="h-11 flex-1" onClick={() => { setLeft(emptyHand()); setRight(emptyHand()); setFeedback(null); }}>Temizle</Button><Button className="h-11 flex-1" onClick={checkAnswer}>Kontrol et</Button></div></div>
          )}

          {feedback && <output className={`mx-auto mt-5 block max-w-xl rounded-xl p-4 text-sm ${feedback.correct ? 'bg-emerald-50 text-emerald-900' : 'bg-amber-50 text-amber-900'}`}><strong>{feedback.correct ? 'Doğru' : 'Tekrar dene'}</strong><p className="mt-1">{feedback.message}</p>{feedback.correct && <Button size="sm" className="mt-3" onClick={newQuestion}>Sıradaki soru</Button>}</output>}

          <div className="mx-auto mt-6 grid max-w-xl grid-cols-3 gap-3 border-t border-border pt-5 text-center"><div><p className="text-2xl font-semibold">{stats.total}</p><p className="text-xs text-muted-foreground">Soru</p></div><div><p className="text-2xl font-semibold text-primary">{stats.correct}</p><p className="text-xs text-muted-foreground">Doğru</p></div><div><p className="text-2xl font-semibold text-[#a66c32]">{stats.wrong}</p><p className="text-xs text-muted-foreground">Tekrar</p></div></div>
        </section>
