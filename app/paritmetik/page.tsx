    ...
    return (
      <main className="paritmetik-login grid min-h-screen place-items-center px-5 py-10">
        <section className="w-full max-w-md rounded-3xl border border-border bg-white p-7 shadow-xl md:p-9">
          <div className="mb-8 flex flex-wrap items-center justify-between gap-3"><Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground"><ArrowLeft size={16} /> Akademiye dön</Link>{educatorHandoff && <Link href="/educator" className="text-sm font-semibold text-primary">Eğitmen ekranı</Link>}</div>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-[1280px] px-5 py-7 md:px-9">
        ...
        <section className="paritmetik-stage rounded-3xl border border-border bg-white p-5 shadow-sm md:p-8">
          ...
          {feedback && <output className={`mx-auto mt-5 block max-w-xl rounded-xl p-4 text-sm ${feedback.correct ? 'bg-emerald-50 text-emerald-900' : 'bg-amber-50 text-amber-900'}`}><strong>{feedback.correct ? 'Doğru' : 'Yanlış'}</strong><p className="mt-1">{feedback.message}</p><p className="mt-2 font-semibold">Doğru cevap: {handMode === 'two' ? String(target).padStart(2, '0') : target}</p>{mode === 'press' && <div className="mt-4 grid grid-cols-2 gap-3 rounded-2xl bg-white/70 p-3"><figure className="m-0 text-center"><figcaption className="mb-1 text-xs font-semibold">Doğru desen</figcaption><FingerHand side="left" pattern={visiblePattern.left} /><FingerHand side="right" pattern={visiblePattern.right} /></figure><figure className="m-0 text-center"><figcaption className="mb-1 text-xs font-semibold">Çocuğun yaptığı</figcaption><FingerHand side="left" pattern={left} /><FingerHand side="right" pattern={right} /></figure></div>}{mode === 'read' && <div className="mt-4 grid grid-cols-2 gap-3 rounded-2xl bg-white/70 p-3 text-center"><div><p className="text-xs font-semibold">Doğru cevap</p><p className="mt-1 text-2xl font-bold">{target}</p></div><div><p className="text-xs font-semibold">Girilen cevap</p><p className="mt-1 text-2xl font-bold">{answer || '—'}</p></div></div>}{transitionMs > 0 ? <p className="mt-2 text-xs">Yeni soru {transitionMs / 1000} saniye içinde otomatik açılacak.</p> : <Button size="sm" className="mt-3" onClick={newQuestion}>Devam</Button>}</output>}
