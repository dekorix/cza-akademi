...
function route(fetch) {
  ...
  new Function('require', 'module', 'exports', 'fetch', js)(id => {
    ...
    return {
      authUrl: p => `https://auth.test${p}`,
      createLocalEducatorSession: async () => ({ user: { email: 'educator@test' }, cookieValue: 'local-session' }),
      educatorCookie: () => 'session=test',
    };
  }, module, module.exports, fetch);
  return module.exports.POST;
}

const request = body => new Request('https://cza.test/api/educator-auth', {
  method: 'POST',
  headers: { origin: 'https://cza.test' },
...
test('reset forwards trusted origin and creates a local educator session', async () => {
  ...
  assert.equal(response.headers.get('set-cookie'), 'session=test');
  const body = await response.json();
  assert.equal(body.ok, true);
  assert.equal(body.signedIn, true);
  assert.equal(body.build, '2026-09-12-reset-autologin-v1');
});
