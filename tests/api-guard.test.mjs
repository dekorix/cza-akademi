...
test('sensitive API routes use a shared request guard', () => {
  assert.match(
    core,
    /await allowRequest\([\s\S]{0,120}'student-login',[\s\S]{0,80}10 \* 60 \* 1000/,
  );
  assert.match(
    report,
    /await allowRequest\(request, 'educator-report', 20, 10 \* 60 \* 1000\)/,
  );
  assert.match(guard, /cza_consume_rate_limit/);
  assert.match(guard, /status: unavailable \? 503 : 429/);
