...
export function readDemoResults(): SessionResult[] {
  try {
    const values: unknown = JSON.parse(localStorage.getItem(RESULT_KEY) || sessionStorage.getItem(RESULT_KEY) || '[]');
...
export function saveDemoResult(result: SessionResult): boolean {
  try { localStorage.setItem(RESULT_KEY, JSON.stringify([result, ...readDemoResults()].slice(0,30))); return true; } catch { return false; }
}
export function saveDemoProgram(config: ExerciseConfig): boolean {
  validateConfig(config);
  try { localStorage.setItem(PROGRAM_KEY, JSON.stringify(config)); return true; } catch { return false; }
}
export function readDemoProgram(): ExerciseConfig | null {
  try { const config = JSON.parse(localStorage.getItem(PROGRAM_KEY) || sessionStorage.getItem(PROGRAM_KEY) || 'null'); if (!config) return null; validateConfig(config); return config; } catch { return null; }
}
