...
const socialEmotionTasks = sectionTasks('social_emotion_play', [
  { phase: 'WARMUP', title: 'Duyguyu Tanı', childInstruction: 'Sence bu çocuk nasıl hissediyor?', educatorInstruction: 'Mutlu/üzgün gibi belirgin tek yüz ifadesiyle başla. Tek sözcük, işaret veya yüz ifadesiyle yanıtı kabul et.', educatorFollowUp: 'Yanıt belirsizse “Yüzünde ne görüyorsun?” diye içerik vermeyen tek takip sorusu sor; kültürel ifade farkını hata sayma.', visualSpec: 'Foto-gerçekçi, yaşa uygun çocuk yüzü; tek belirgin duygu, nötr arka plan, abartılı karikatür mimik yok.', responseMode: 'SPEAK', evidenceFocus: ['duygu tanıma', 'sosyal ipucu', 'ilk yanıt'] },
  { phase: 'CORE', title: 'Sıra Bende-Sende', childInstruction: 'Bir sen koy, bir ben koyayım.', educatorInstruction: 'Kısa blok oyununda 3–4 tur sıra alma fırsatı ver. Hatırlatma ve bağımsız sıra alma davranışını ayır.', educatorFollowUp: 'Sırasını beklerken bakış, eli geri çekme, “benim sıram” deme gibi düzenleme davranışlarını not et.', visualSpec: 'Ekran gerekmez; 6–8 büyük blok ve ortak oyun alanı. Oyuncaklar rekabeti artırmayacak kadar sade.', responseMode: 'OBSERVE', materials: ['büyük bloklar'], evidenceFocus: ['sıra alma', 'ortak dikkat', 'bekleme'] },
  { phase: 'DEEPEN', title: 'Rol Oyunu Kur', childInstruction: 'Bu bebeğin doktoru olsak ne yapardık?', educatorInstruction: 'Tanıdık bakım rolü üzerinden sembolik oyun başlat. Tek bir doğru senaryo bekleme; çocuğun nesneye anlam yüklemesini gözle.', educatorFollowUp: 'Kendiliğinden rol, taklit, nesneyi sembolik kullanma ve yetişkinden fikir isteme biçimini ayrı kaydet.', visualSpec: 'Gerçek bebek/figür ve sade oyuncak doktor materyali; korkutucu tıbbi ayrıntı yok.', responseMode: 'OBSERVE', materials: ['bebek veya figür', 'oyuncak doktor seti'], evidenceFocus: ['sembolik oyun', 'rol üretimi', 'ortak oyun'] },
  { phase: 'STRATEGY', title: 'Arkadaş Üzgünse', childInstruction: 'Arkadaşın üzülse ne yapabilirsin?', educatorInstruction: 'Açık uçlu sosyal problem sun. Teselli, yardım çağırma, yanında durma gibi farklı işlevsel çözümleri kabul et; tek doğru cevap arama.', educatorFollowUp: '“Başka ne olabilir?” sorusunu en fazla bir kez kullan; çözümün işlevini ve çocuğun gerekçesini not et.', visualSpec: 'Bir çocuğun oyuncağı kırıldığı için üzgün olduğu sade sahne; metin balonu veya cevabı yönlendiren yetişkin yok.', responseMode: 'SPEAK', evidenceFocus: ['sosyal problem çözme', 'empati tepkisi', 'alternatif üretme'] },
  { phase: 'LEARNING_RESPONSE', title: 'Oyuna Katılmayı Gör ve Dene', childInstruction: 'Bak, ben “Ben de oynayabilir miyim?” diye soracağım. Şimdi başka bir oyunda sen nasıl katılacağını dene.', educatorInstruction: 'Önce bağımsız sosyal giriş fırsatı ver. Zorlanırsa farklı bir oyun sahnesinde tek model göster; ardından YENİ sahnede yeniden denet.', educatorFollowUp: 'Model öncesi/sonrası değişimi kaydet: yaklaşmadı / modeli taklit etti / yeni sahnede kendi ifadesini üretti.', visualSpec: 'Model sahnesi: blok oynayan iki çocuk; test sahnesi: top oynayan iki çocuk. Gerçekçi ve sıcak ama cevabı yazıyla göstermeyen görseller.', responseMode: 'SPEAK', evidenceFocus: ['öğrenmeye tepki', 'sosyal başlatma', 'modelden öğrenme', 'destek sonrası değişim'] },
  { phase: 'NEAR_TRANSFER', title: 'Farklı Tercihi Anla', childInstruction: 'Sen dondurmayı seviyorsun; arkadaşın sevmeyebilir mi?', educatorInstruction: '39+ ay için farklı tercih olabileceğini doğal sohbetle sor. “Evet” demeyi tek başına yeterli kanıt sayma; mümkünse kısa açıklama al.', educatorFollowUp: 'Çocuk kendi tercihini tek doğru gibi savunursa bunu yargılamadan bakış açısı kanıtı olarak not et; doğru cevabı öğretmeye çalışma.', visualSpec: 'İki çocuk ve farklı yiyecek tercihlerine işaret eden sade sahne; yüz ifadeleri nötr/olumlu, çatışma yok.', responseMode: 'SPEAK', evidenceFocus: ['bakış açısı', 'yakın transfer', 'farklı tercih kabulü'] },
  { phase: 'FAR_TRANSFER', title: 'Oyun Kuralı Değişirse', childInstruction: 'Oyunda sıra değişirse ne yapabiliriz?', educatorInstruction: '42+ ay için sosyal esneklik ve regülasyon senaryosu sun. Birden fazla kabul edilebilir çözüm vardır.', educatorFollowUp: 'Bekleme, yeniden sorma, başka oyuna geçme veya yetişkinden yardım isteme gibi seçenekleri işlevsel sosyal strateji olarak not et.', visualSpec: 'Sıra bekleyen çocukların olduğu basit masa oyunu sahnesi; kazanan-kaybeden vurgusu ve kızgın yetişkin yok.', responseMode: 'SPEAK', evidenceFocus: ['sosyal esneklik', 'regülasyon', 'uzak transfer', 'problem çözme'] },
  { phase: 'CEILING', title: 'Karakter Ne Düşünüyor?', childInstruction: 'Sence bu karakter ne düşünüyor olabilir?', educatorInstruction: '45+ ay nötr keşif/tavan görevidir. Birden fazla olası zihinsel duruma açık sahne kullan; tek doğru zihinsel durum atfetme.', educatorFollowUp: 'Yanıtı doğru/yanlış olarak değil, sahnedeki ipuçlarıyla ilişki kurma biçimi olarak kaydet. Yanıt vermemesi alan puanını düşürmez.', visualSpec: 'Bir çocuğun kapalı kutuya bakıp merak ettiği belirsiz ama güvenli sahne; düşünce balonu veya cevabı veren metin yok.', responseMode: 'SPEAK', evidenceFocus: ['zihinsel durum çıkarımı', 'sosyal ipucu birleştirme', 'nötr tavan'] },
]);
const motorTasks = sectionTasks('motor_graphomotor', [
  { phase: 'WARMUP', title: 'Boncukları İpe Diz', childInstruction: 'Bu boncukları ipe dizmeyi dener misin?', educatorInstruction: 'Büyük boncuk ve kalın ip kullan. İlk yaklaşımı, iki elin birlikte kullanımını ve yardımı kaydet; hız baskısı kurma.', educatorFollowUp: 'İpi sabitleme, boncuğu çevirme veya el değiştirme gibi kendiliğinden stratejiyi not et. Başaramazsa tek sözel ipucu verilebilir.', visualSpec: 'Gerçek büyük boncuklar ve kalın ip; yüksek kontrastlı, güvenli masa düzeni. Ekran simülasyonu yerine gerçek materyal.', responseMode: 'OBSERVE', materials: ['kalın ip', 'büyük boncuklar'], evidenceFocus: ['iki el koordinasyonu', 'ince motor başlangıç', 'kendiliğinden strateji'] },
  { phase: 'CORE', title: 'Büyük Topu Yakala', childInstruction: 'Topu sana yavaşça atacağım. Yakalamayı dener misin?', educatorInstruction: 'Büyük yumuşak topu kısa mesafeden, güvenli hızda 2 doğal denemeyle sun. Başarı sayısından çok hazırlık, uzanma ve iki el koordinasyonunu gözle.', educatorFollowUp: 'Topu gövdeye alma, elleri birlikte uzatma veya kaçınma gibi motor yaklaşımı kısa notla kaydet.', visualSpec: 'Büyük yumuşak top, boş ve güvenli alan; kaygan zemin veya sert nesne bulunmamalı.', responseMode: 'OBSERVE', materials: ['büyük yumuşak top'], evidenceFocus: ['kaba motor koordinasyon', 'görsel-motor zamanlama', 'iki el kullanımı'] },
  { phase: 'DEEPEN', title: 'Çizgiyi Benim Gibi Yap', childInstruction: 'Benim gibi yukarıdan aşağı bir çizgi çizer misin?', educatorInstruction: 'Bir kez kısa model göster. Kalem tutuşunu görev sırasında zorla düzeltme; başlangıç yönü, süreklilik ve görsel-motor taklidi kaydedilir.', educatorFollowUp: 'Çizgiyi parça parça mı tek hareketle mi yaptı, kağıdı diğer eliyle sabitledi mi, model sonrası davranış değişti mi not et.', visualSpec: 'Büyük beyaz kağıt, kalın boya kalemi ve tek koyu dikey örnek çizgi; gereksiz şekil veya yazı yok.', responseMode: 'DRAW', materials: ['kalın boya kalemi', 'kağıt'], evidenceFocus: ['grafomotor taklit', 'çizgi yönü', 'iki el organizasyonu'] },
  { phase: 'STRATEGY', title: 'Yuvarlağı Nasıl Çizersin?', childInstruction: 'Bu yuvarlağa bak. Sen de bir benzerini çizer misin?', educatorInstruction: 'Mükemmel geometrik şekil bekleme. Başlangıç noktası, kapanışa yaklaşma, kalemi kaldırma ve sayfayı kullanma stratejisini gözle.', educatorFollowUp: 'Gerekirse yalnız “Nasıl yapmayı düşündün?” diye sor; sözel açıklama şart değildir. El hareketi ve yeniden deneme de strateji kanıtıdır.', visualSpec: 'Tek büyük koyu daire örneği ve yanında geniş boş alan; küçük detay ve dekor yok.', responseMode: 'DRAW', materials: ['kalın kalem', 'kağıt'], evidenceFocus: ['şekil taklidi', 'grafomotor strateji', 'öz-düzeltme'] },
  { phase: 'LEARNING_RESPONSE', title: 'Yolu Gör ve Yeni Yolda Dene', childInstruction: 'Önce ben bu yolu çizeceğim. Sonra sen başka bir yolu dene.', educatorInstruction: 'Önce kısa bağımsız bir yol izleme fırsatı ver. Zorlanırsa farklı bir örnek üzerinde tek model göster; ardından YENİ bir yol ver ve model sonrası değişimi kaydet.', educatorFollowUp: 'Model öncesi ve sonrası çizgi dışına taşma, yönü koruma ve durup yeniden başlama davranışını ayrı yaz. Aynı örneği ezberletme.', visualSpec: 'Kalın ve geniş iki basit yol: öğretim için düz-hafif kıvrımlı yol, test için farklı kıvrımlı yeni yol; başlangıç ve bitiş sembolleri sade.', responseMode: 'DRAW', materials: ['kalın kalem', 'iki farklı yol sayfası'], evidenceFocus: ['modelden öğrenme', 'görsel-motor kontrol', 'destek sonrası değişim'] },
  { phase: 'NEAR_TRANSFER', title: 'Büyük Düğmeyi Aç', childInstruction: 'Bu büyük düğmeyi açmayı dener misin?', educatorInstruction: '39+ ay için büyük düğmeli çerçeve kullan. İki eli birlikte kullanma, kumaşı sabitleme ve motor planlamayı gözle; fiziksel yardımı açıkça kaydet.', educatorFollowUp: 'Önceki iki-el stratejisinin yeni günlük materyale taşınıp taşınmadığını; çekme, itme, çevirme denemelerini not et.', visualSpec: 'Büyük, kolay kavranan tek düğmeli kumaş çerçeve; fermuar veya başka kapama türü aynı anda bulunmamalı.', responseMode: 'OBSERVE', materials: ['büyük düğmeli çerçeve'], evidenceFocus: ['yakın transfer', 'ince motor planlama', 'iki el koordinasyonu', 'özbakım motoru'] },
  { phase: 'FAR_TRANSFER', title: 'Bir İnsan Çiz', childInstruction: 'Bir insan çizer misin?', educatorInstruction: '42+ ay için açık uçlu temsili çizim kullan. Parça sayısını norm puanına çevirmeden; planlama, sayfa kullanımı, çizgi kontrolü ve temsil stratejisini betimle.', educatorFollowUp: '“Bitirdim” dediğinde yalnız “Bana çizimini anlatmak ister misin?” diye sor. Ek parça çizmesini telkin etme; spontan eklemeleri ayrı not et.', visualSpec: 'Boş büyük kağıt ve tek kalem; örnek insan resmi gösterilmez, kopyalama görevi değildir.', responseMode: 'DRAW', materials: ['kalem', 'kağıt'], evidenceFocus: ['temsili çizim', 'uzak transfer', 'grafomotor planlama', 'spontan organizasyon'] },
  { phase: 'CEILING', title: 'Kalın Yolu Makasla Takip Et', childInstruction: 'Bu kalın yolu makasla takip etmeyi dener misin?', educatorInstruction: '45+ ay nötr keşif/tavan görevidir. Yalnız çocuk makası ve yakın yetişkin gözetimi kullan. Başarısızlık alan puanını düşürmez.', educatorFollowUp: 'Kağıdı çevirme, makası yeniden konumlandırma, iki elin görev paylaşımı ve güvenli durma davranışını betimle; hız isteme.', visualSpec: 'Çocuk makasına uygun kalın, kısa ve hafif kıvrımlı kesme yolu; keskin köşe, küçük parça veya karmaşık desen yok.', responseMode: 'OBSERVE', materials: ['çocuk makası', 'kalın çizgili kağıt'], evidenceFocus: ['görsel-motor planlama', 'iki el koordinasyonu', 'araç kullanımı', 'nötr tavan'] },
]);
const dailyLivingTasks = sectionTasks('daily_living_safety', [
  { phase: 'WARMUP', title: 'Oyuncağın Yerini Bul', childInstruction: 'Oyuncağın işi bitti. Sence şimdi nereye koyabiliriz?', educatorInstruction: 'Tanıdık gerçek ortamda tek oyuncak ve belirgin saklama yeri kullan. İlk davranışı, hatırlatma gereksinimini ve rutine katılımı kaydet.', educatorFollowUp: 'Evde farklı bir düzen varsa bunu hata sayma; çocuğun kendi rutinini göstermesine izin ver.', visualSpec: 'Gerçek oyuncak ve erişilebilir oyuncak kutusu/raf; gereksiz seçenek ve yazı yok.', responseMode: 'OBSERVE', materials: ['tanıdık oyuncak', 'oyuncak kutusu veya raf'], evidenceFocus: ['günlük rutin', 'bağımsız başlatma', 'hatırlatma düzeyi'] },
  { phase: 'CORE', title: 'Montumu Giyiyorum', childInstruction: 'Montunu giymeyi dener misin?', educatorInstruction: 'Süre baskısı kurmadan gerçek giysi kullan. Hangi aşamayı bağımsız yaptığı, nerede yardım istediği ve motor/planlama yaklaşımını kaydet.', educatorFollowUp: 'Evde bu beceriyi deneme fırsatı azsa bunu not et; fırsat eksikliğini çocuk yetersizliği olarak yorumlama.', visualSpec: 'Çocuğa uygun kendi montu veya kolay giyilen benzer giysi; düğme/fermuar zorunlu başarı ölçütü değildir.', responseMode: 'OBSERVE', materials: ['mont veya kolay giyilen üst giysi'], evidenceFocus: ['giyinme bağımsızlığı', 'sıra', 'yardım düzeyi'] },
  { phase: 'DEEPEN', title: 'Ellerimi Temizliyorum', childInstruction: 'Ellerimizi temizlemek için neler yapabiliriz?', educatorInstruction: 'Mümkünse gerçek lavabo/rutin üzerinden, değilse görsel/nesne sırasıyla gözle. Tek ezber cümlesi yerine eylem sırasını ve günlük yaşam bilgisini kaydet.', educatorFollowUp: 'Çocuğun evdeki rutini farklıysa ayrıntıyı bakımveren kaynağıyla bağlamlandır; hijyen pratiğini ahlaki değerlendirmeye dönüştürme.', visualSpec: 'Lavabo, sabun ve havluya ait sade gerçekçi görseller ya da gerçek ortam; korkutucu mikrop görseli yok.', responseMode: 'OBSERVE', materials: ['lavabo/sabun/havlu veya rutin kartları'], evidenceFocus: ['özbakım rutini', 'eylem sırası', 'günlük yaşam bilgisi'] },
  { phase: 'STRATEGY', title: 'Olmuyorsa Nasıl Yardım İstersin?', childInstruction: 'Bu kutuyu açamazsan ne yapabilirsin?', educatorInstruction: 'Kolayca açılmayan ama güvenli bir kap kullan; kısa bağımsız denemeden sonra yardım arama stratejisini gözle. Çocuğu uzun süre frustrasyonda bırakma.', educatorFollowUp: 'Sözcük, işaret, yetişkine bakma veya nesneyi uzatma gibi işlevsel yardım isteme yollarını kabul et ve biçimini not et.', visualSpec: 'Güvenli, keskin olmayan, çocuk için zor ama yetişkin için kolay açılan kap; içinde cazip ama zorunlu ödül yok.', responseMode: 'OBSERVE', materials: ['güvenli kapaklı kap'], evidenceFocus: ['yardım isteme', 'problem çözme stratejisi', 'iletişim işlevi'] },
  { phase: 'LEARNING_RESPONSE', title: 'Yeni Rutini Gör ve Uygula', childInstruction: 'Önce ben bir kez yapacağım. Sonra başka eşyalarla sen dener misin?', educatorInstruction: 'Önce kısa bağımsız düzenleme denemesi ver. Zorlanırsa farklı nesnelerle iki aşamalı “yerine koy + kapağı kapat” modelini bir kez göster; ardından YENİ nesnelerle tekrar denet.', educatorFollowUp: 'Model öncesi ve sonrası değişimi ayrı kaydet: rutini başlatma, sırayı koruma, yardım gereksinimi ve yeni nesnelere aktarım.', visualSpec: 'İki farklı günlük eşya seti ve kapaklı/kapaksız saklama alanı; model seti ile test seti farklı olmalı.', responseMode: 'OBSERVE', materials: ['iki farklı eşya seti', 'saklama kutusu'], evidenceFocus: ['modelden öğrenme', 'günlük rutin öğrenme', 'destek sonrası değişim'] },
  { phase: 'NEAR_TRANSFER', title: 'Yetişkinini Bulamazsan', childInstruction: 'Bir yerde yetişkinini göremezsen ne yapabilirsin?', educatorInstruction: '39+ ay için sakin ve korkutmayan bir güvenlik senaryosu kullan. Tek ezber cümle arama; çocuğun yardım arama planını ve güvenli yerde kalma fikrini dinle.', educatorFollowUp: '“Güvenilir bir yetişkinden yardım isteme”, “olduğun yerde bekleme” gibi işlevsel seçenekleri kabul et; aile planı farklıysa bakımveren bilgisiyle bağlamlandır.', visualSpec: 'Kalabalık olmayan güvenli bir kamusal alan sahnesi; çocuk sakin, tehlike veya kaçırılma imgesi yok.', responseMode: 'SPEAK', evidenceFocus: ['yakın transfer', 'yardım arama planı', 'güvenlik muhakemesi'] },
  { phase: 'FAR_TRANSFER', title: 'İki Aşamalı Günlük Rutin', childInstruction: 'Önce mendili al, sonra çöpe at.', educatorInstruction: '42+ ay için günlük yaşam içinde iki aşamalı, mahremiyet içermeyen rutin uygulat. İçerik bilgisi ile sırayı sürdürmeyi ayrı kaydet.', educatorFollowUp: 'Yönerge tekrarı gerekirse destek düzeyi olarak kaydet. Gerçek kirli mendil yerine temiz/temsilî materyal kullan.', visualSpec: 'Temiz mendil ve erişilebilir küçük çöp kutusu; hijyenik ve sade ortam.', responseMode: 'OBSERVE', materials: ['temiz mendil', 'çöp kutusu'], evidenceFocus: ['uzak transfer', 'özbakım sırası', 'bağımsızlık', 'yönergeyi rutine çevirme'] },
  { phase: 'CEILING', title: 'Güvenli Seçimi Açıkla', childInstruction: 'Bu iki durumdan hangisi daha güvenli olabilir? İstersen nedenini de söyle.', educatorInstruction: '45+ ay nötr keşif/tavan görevidir. Grafik korku veya gerçek tehlike kullanmadan iki günlük durum göster; tek gerekçe kalıbı bekleme ve başarısızlığı alan puanına katma.', educatorFollowUp: 'Seçim kadar gerekçenin sahne ipuçlarıyla ilişkisini kaydet. Aile kuralı farklıysa bunu “yanlış” diye etiketleme.', visualSpec: 'Örneğin oyuncakların toplandığı açık yürüyüş yolu ile yerde oyuncakların olduğu yol gibi iki güvenli günlük sahne; yaralanma görüntüsü yok.', responseMode: 'SPEAK', evidenceFocus: ['güvenlik muhakemesi', 'gerekçe', 'bağlama duyarlı karar', 'nötr tavan'] },
]);
const learningTransferTasks = sectionTasks('learning_transfer', [
  { phase: 'WARMUP', title: 'Benim Gibi Yap', childInstruction: 'Ben bir kez yapacağım, sonra sen dener misin?', educatorInstruction: 'İki parçalı çok basit bir blok yapısını bir kez modelle. Taklidin doğruluğundan çok modeli izleme, başlatma ve yardım gereksinimini kaydet.', educatorFollowUp: 'Modeli tekrar etmeden önce ilk denemeyi kaydet. Parça yerini farklı seçse bile temel ilişkiyi koruyorsa bunu yaklaşık başarı/strateji olarak not et.', visualSpec: 'İki büyük blokla basit üst üste veya yan yana yapı; çocuğun test blokları aynı türde ama farklı renkte olabilir.', responseMode: 'MANIPULATIVE', materials: ['2-3 büyük blok'], evidenceFocus: ['modeli izleme', 'taklit yoluyla öğrenme', 'bağımsız başlatma'] },
  { phase: 'CORE', title: 'İlk Yol Olmazsa', childInstruction: 'Bu parça buraya olmuyorsa başka ne deneyebiliriz?', educatorInstruction: 'Şekil kutusunda bilerek ilk denemeye uymayan güvenli bir parça seç. Çocuğun aynı yolu tekrarlama, parçayı çevirme veya başka açıklık deneme stratejisini gözle.', educatorFollowUp: 'Yetişkin çözümü söylemeden önce kaç farklı strateji denediğini ve yardım arayıp aramadığını kısa notla kaydet.', visualSpec: 'Büyük güvenli şekil kutusu; 2-3 belirgin açıklık ve kolay kavranan tek parça. Aşırı çeldirici yok.', responseMode: 'MANIPULATIVE', materials: ['şekil kutusu'], evidenceFocus: ['strateji değiştirme', 'ısrar-esneklik dengesi', 'problem çözme'] },
  { phase: 'DEEPEN', title: 'Küçük İpucunu Kullan', childInstruction: 'Sana küçük bir ipucu vereceğim. Sonra yeniden dener misin?', educatorInstruction: 'Önce bağımsız denemeyi kaydet. Sonra çözümü söylemeyen tek bir sözel veya görsel ipucu ver; ipucu sonrası davranış değişimini ayrı kaydet.', educatorFollowUp: 'Değişmedi / aynı stratejiyi sürdürdü / ipucunu kullandı / kendi yeni yolunu buldu ayrımını notta açık yaz.', visualSpec: 'Basit eşleme ya da yerleştirme görevi; ipucu cevabı işaret etmemeli, yalnız ilgili özelliğe dikkat çekmeli.', responseMode: 'OBSERVE', evidenceFocus: ['ipucuna tepki', 'öğrenme eğimi', 'öz-düzeltme', 'destek sonrası değişim'] },
  { phase: 'STRATEGY', title: 'Başlamadan Önce Planla', childInstruction: 'Bunu yapmaya başlamadan önce ne yapacağını gösterebilir ya da söyleyebilir misin?', educatorInstruction: 'Kısa plan davranışı yeterlidir; yetişkin dili bekleme. İşaret, parçayı seçme veya sözel planı kabul et ve uygulamayla tutarlılığını gözle.', educatorFollowUp: 'Plan söyleyemese bile eyleme geçmeden çevreyi tarama, parça seçme veya deneme sırasını strateji olarak kaydet.', visualSpec: '3 parçalı basit yapım/yerleştirme görevi; parçalar görünür ama önceden yerleştirilmemiş.', responseMode: 'OBSERVE', materials: ['3 parçalı basit görev'], evidenceFocus: ['planlama', 'stratejiyi görünür kılma', 'üstbiliş başlangıcı'] },
  { phase: 'LEARNING_RESPONSE', title: 'Yeni Kuralı Öğren ve Yeniden Dene', childInstruction: 'Önce sen dene. Sonra ben sana kuralı göstereceğim; ardından başka resimlerle tekrar deneyelim.', educatorInstruction: 'Bağımsız başlangıç örneğini kaydet. Gerekirse tek açık model ile yeni bir sınıflama/eşleme kuralı öğret; modelden SONRA farklı örneklerle yeniden denet.', educatorFollowUp: 'Model öncesi → model sonrası → yeni örnek performansını üç ayrı kısa cümleyle kaydet; öğrenme hızını tek doğru/yanlış sonucuna indirgeme.', visualSpec: 'Öğretim ve test için yüzey özellikleri farklı iki sade kart seti; aynı ilişki kuralı korunur, aynı kartlar tekrar kullanılmaz.', responseMode: 'MANIPULATIVE', materials: ['iki farklı kart/nesne seti'], evidenceFocus: ['öğrenmeye tepki', 'modelden öğrenme', 'öğrenme eğimi', 'yeni örneğe geçiş'] },
  { phase: 'NEAR_TRANSFER', title: 'Aynı Kural, Yeni Malzeme', childInstruction: 'Şimdi aynı oyunu başka oyuncaklarla yapalım. Nasıl yaparsın?', educatorInstruction: '39+ ay için öğrenilmiş kuralı yeni materyale taşı. Kuralı yeniden söylemeden önce bağımsız denemeyi kaydet; gerekirse sonra kısa hatırlatma ver.', educatorFollowUp: 'Yüzey değişince kuralı korudu mu, yalnız eski nesnelere mi bağlı kaldı, hatırlatma sonrası toparladı mı not et.', visualSpec: 'Önceki görevden tamamen farklı görünümlü ama aynı ilişki kuralını taşıyan yeni oyuncak/kart seti.', responseMode: 'OBSERVE', materials: ['yeni materyal seti'], evidenceFocus: ['yakın transfer', 'kural koruma', 'genelleme'] },
  { phase: 'FAR_TRANSFER', title: 'Başka Bir Çözüm Bul', childInstruction: 'Bunu başka bir yoldan da yapabilir miyiz?', educatorInstruction: '42+ ay için tek çözümü olmayan basit problem kullan. İlk çözümden sonra farklı bir yol üretme veya yetişkin önerisini yeni duruma uyarlama becerisini gözle.', educatorFollowUp: 'İkinci yolun tamamen farklı olması şart değildir; anlamlı strateji değişimi, malzeme değişimi veya yeni sıra da alternatif çözüm kanıtıdır.', visualSpec: 'Bir hedefe iki farklı basit yoldan ulaşılabilen blok/yerleştirme problemi; tek doğru çözüm görüntüsü verilmez.', responseMode: 'MANIPULATIVE', materials: ['çok çözümlü basit problem seti'], evidenceFocus: ['uzak transfer', 'alternatif üretme', 'esneklik', 'strateji çeşitliliği'] },
  { phase: 'CEILING', title: 'Bu Kural Başka Nerede İşe Yarar?', childInstruction: 'Bu öğrendiğimiz şey başka nerede işimize yarayabilir?', educatorInstruction: '45+ ay nötr keşif/tavan görevidir. Yakın örnek vermeden çocuğun kendi genellemesini bekle; yanıt verememesi alan puanını düşürmez.', educatorFollowUp: 'Sözel örnek, işaret ettiği benzer durum veya materyalle spontan gösterim de uzak transfer kanıtıdır. Tek “doğru” kullanım bekleme.', visualSpec: 'Önceki görevin materyali görünür olabilir ancak yeni kullanım cevabını gösteren ek görsel bulunmaz.', responseMode: 'SPEAK', evidenceFocus: ['uzak transfer keşfi', 'genelleme', 'üstbiliş', 'nötr tavan'] },
]);
