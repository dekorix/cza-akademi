export const CZA_CONTRACT_VERSION = '1.1.0';
export const CZA_LEGACY_CONTRACT_VERSION = '1.0.0';
export const CZA_SCHEMA_VERSION = 'CZA_MODULE_RECORD_V1';

const ACCEPTED_CONTRACT_VERSIONS = new Set([
  CZA_LEGACY_CONTRACT_VERSION,
...
const MAX_RECORD_DURATION_MS = 24 * 60 * 60 * 1000;
...
export type CanonicalLearningRecord = {
  ...
  contractVersion:
    | typeof CZA_CONTRACT_VERSION
    | typeof CZA_LEGACY_CONTRACT_VERSION;
  ...
  if (isPlainObject(value)) {
    ...
    for (const [key, item] of entries) {
      if (
        ...
        fail('cza_record_json_key_invalid');
      }
...
export function parseCanonicalLearningRecord(
  ...
  if (value.schemaVersion !== CZA_SCHEMA_VERSION)
    fail('cza_schema_version_invalid');
  if (
    typeof value.contractVersion !== 'string' ||
    !ACCEPTED_CONTRACT_VERSIONS.has(value.contractVersion)
  )
    fail('cza_contract_version_invalid');
  if (requestContractVersion !== value.contractVersion) {
    fail('cza_contract_header_mismatch');
  }

  const clientRecordId = text(
    value.clientRecordId,
    'cza_client_record_id_invalid',
    100,
  );
  const trainingSessionId = text(
  ...
  if (
    ...
    completedTime - startedTime > MAX_RECORD_DURATION_MS ||
    startedTime > Date.now() + MAX_CLOCK_SKEW_MS ||
    completedTime > Date.now() + MAX_CLOCK_SKEW_MS
  ) {
    fail('cza_record_time_order_invalid');
  }

  const supportLevel = text(
    value.supportLevel,
    'cza_support_level_invalid',
    32,
  );
  ...
  return {
    ...
    contractVersion:
      value.contractVersion as CanonicalLearningRecord['contractVersion'],
    clientRecordId,
    trainingSessionId,
    moduleId,
    moduleVersion,
